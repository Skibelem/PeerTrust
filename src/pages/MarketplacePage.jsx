...
// ─── RLS hint block (reused for trade errors) ─────────────────────────────────
    ...
    <div className="bg-red-50 border border-red-200 rounded-2xl p-5 flex gap-3">
      ...
      <div className="min-w-0">
        ...
        <div className="mt-3 p-3 bg-red-100 rounded-xl text-xs text-red-800 font-mono leading-relaxed">
          ...
          <pre className="whitespace-pre-wrap">{forTrades ? `-- Buyers can insert trades
...
ON public.trades FOR SELECT
USING (auth.uid() = seller_id);` : `-- Allow all logged-in users to read active offers
...
function OfferCard({ offer, profile, onTradeError }) {
  ...
  async function handleStartTrade() {
    setStarting(true)
    onTradeError(null) // clear previous error
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`shrink-0 text-[11px] font-semibold px-2.5 py-0.5 rounded-full border capitalize ${categoryStyle(offer.category)}`}>
          {offer.category}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-50 mt-auto">
        <span className="text-xl font-extrabold text-slate-900">
          {formatNGN(offer.price)}
        </span>

        {/* Seller: cannot start trade */}
        {isSeller && (
          <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed select-none">
        ...
        {!isSeller && (
          <button
            ...
            disabled={starting}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-teal-600 hover:bg-teal-700 disabled:bg-teal-300 disabled:cursor-not-allowed text-white transition-colors"
          >
            {starting ? (
              <>
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 py-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Service Marketplace</h1>
            <p className="text-slate-500 text-sm mt-1">
              {isSeller
                ? 'Browse seller offers. Sellers cannot start trades.'
                : 'Find a seller and start a trade instantly.'}
            </p>
          </div>
          {isSeller && (
            <Link
        ...
        {/* Trade error */}
        {tradeError && <RlsHint error={tradeError} forTrades={true} />}
