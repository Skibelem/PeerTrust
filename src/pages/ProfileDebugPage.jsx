...
import { Shield, ArrowLeft, Bug, Server, User, Wallet, Terminal, CheckCircle } from 'lucide-react'
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-extrabold text-slate-900">Supabase Connection Sandbox</h2>
        </div>

        {error && (
          <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg shadow-sm">
            <h4 className="font-bold text-red-800 text-sm">Supabase Fetch Failure Detected</h4>
        ...
        {profileMissing && (
          <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-r-lg shadow-sm">
            <h4 className="font-bold text-yellow-800 text-sm">Database Row Missing</h4>
            <p className="text-xs text-yellow-700 mt-1">
              Auth session exists, but no database rows exist under table `public.profiles` for ID: <code className="bg-yellow-100 font-mono p-0.5 rounded text-[10px]">{user?.id}</code>.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <span className="text-slate-400 font-semibold uppercase tracking-wider block text-[10px]">Auth User ID</span>
              ...
              <div>
                <span className="text-slate-400 font-semibold uppercase tracking-wider block text-[10px]">Active Email</span>
                <span className="text-slate-900 font-medium">{user?.email || 'null'}</span>
              </div>

              <div>
                <span className="text-slate-400 font-semibold uppercase tracking-wider block text-[10px]">Session Provider metadata</span>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <span className="text-slate-400 font-semibold uppercase tracking-wider block text-[10px]">Profile Name</span>
                <span className="text-slate-900 font-bold">{profile?.full_name || 'null'}</span>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <span className="text-slate-400 font-semibold uppercase tracking-wider block text-[10px]">Profile Role</span>
                ...
                <div>
                  <span className="text-slate-400 font-semibold uppercase tracking-wider block text-[10px]">Profile Status</span>
              ...
              <div>
                ...
                <div className="mt-1 bg-slate-50 border border-slate-100 p-2.5 rounded-lg space-y-1 font-mono text-[10px] text-slate-700">
                  <div className="flex justify-between">
                    <span>Available:</span>
                    <span className="font-bold text-slate-950">{formatCurrency(wallet?.available_balance)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Escrow Balance:</span>
                    <span className="font-bold text-slate-950">{formatCurrency(wallet?.escrow_balance)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Pending Withdrawals:</span>
                    <span className="font-bold text-slate-950">{formatCurrency(wallet?.pending_withdrawal)}</span>
                  </div>
                </div>
              </div>
            </div>
